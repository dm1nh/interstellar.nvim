...
local get_compiled_path = function(theme)
  return table.concat({ vim.fn.stdpath("state"), "interstellar", theme .. "_compiled.lua" }, PATH_SEP)
end

---@return string theme
function M.get_theme_from_bg_opt()
  local config = require("interstellar").config
...
function M.compile(theme, highlights, termcolors)
  vim.loop.fs_mkdir(vim.fn.stdpath("state") .. PATH_SEP .. "interstellar", 448)

  local fname = get_compiled_path(theme)
  local file, err = io.open(fname, "wb")
  if not file or err then
    vim.notify("Interstellar: Error writing " .. fname .. ":\n" .. err, vim.log.levels.ERROR)
    return
  end

  local lines = {
    "require'interstellar'.compiled = string.dump(function()",
    "local g = vim.g",
  ...
  file:write(require("interstellar").compiled)
  file:close()
end
